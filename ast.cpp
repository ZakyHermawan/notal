...
llvm::Type* ast::TypeDecl::toLLVMType() {
    ...
    switch(this->sys_name) {
        case ast::TypeDecl::TypeName::integer:  return llvm::Type::getInt32Ty(TheContext);    break;
        case ast::TypeDecl::TypeName::real:     return llvm::Type::getDoubleTy(TheContext);    break;
        case ast::TypeDecl::TypeName::character:return llvm::Type::getInt8Ty(TheContext);     break;
        case ast::TypeDecl::TypeName::boolean:  return llvm::Type::getInt1Ty(TheContext);     break;
        case ast::TypeDecl::TypeName::range:    return llvm::Type::getInt32Ty(TheContext);    break;
        ...
        default:                                return llvm::Type::getVoidTy(TheContext);     break;
    }
}


// Code Gen section
llvm::Value* ast::Identifier::CodeGen(Codegen& context) {
    ...
    //     return nullptr;
    // }
    return new llvm::LoadInst(context.getValue(name)->getType(), context.getValue(name), "", false, context.currentBlock());
}

llvm::Value* ast::IntegerType::CodeGen(Codegen& context) {
    std::cout << "Creating integer: " << val << std::endl;
	return llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), val, true);
}
llvm::Value* ast::RealType::CodeGen(Codegen& context) {
    std::cout << "Creating real: " << val << std::endl;
    return llvm::ConstantFP::get(TheContext, llvm::APFloat(val));
	//return llvm::ConstantFP::get(llvm::Type::getFloatTy(TheContext), val, true);
}
llvm::Value* ast::CharType::CodeGen(Codegen& context) {
    std::cout << "Creating char: " << val << std::endl;
	return llvm::ConstantInt::get(llvm::Type::getInt8Ty(TheContext), val, true);
}
llvm::Value* ast::BooleanType::CodeGen(Codegen& context) {
    std::cout << "Creating boolean: " << val << std::endl;
	return llvm::ConstantInt::get(llvm::Type::getInt1Ty(TheContext), val, true);
}
llvm::Value* ast::RangeType::CodeGen(Codegen& context) {
    if (this->isNameRange) {
...
llvm::Value* ast::BinaryOperator::CodeGen(Codegen& context) {
...
llvm::Value* ast::AssignmentStmt::CodeGen(Codegen& context) {
...
llvm::Value* ast::ConstDecl::CodeGen(Codegen& context) {
...
llvm::Value* ast::VarDecl::CodeGen(Codegen& context) {
    ...
    if (isGolbal)
    {
        ...
        if (this->type->sys_name  == ast::TypeDecl::TypeName::array) {
            ...
            switch(sub_type) {
                case ast::TypeDecl::TypeName::integer: 
                    DBMSG("into integer");
                    ele_of_arr = llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 0, true);
                    break;
                case ast::TypeDecl::TypeName::real: 
                    DBMSG("into real");
                    ele_of_arr = llvm::ConstantFP::get(llvm::Type::getDoubleTy(TheContext), 0);
                    break;
            }
        ...
        auto go= new llvm::GlobalVariable(*context.module, this->type->toLLVMType(), false, llvm::GlobalValue::ExternalLinkage , llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 0, true), this->name->name);
        alloc = go;
        }
    }
    else
...
llvm::Value* ast::Program::CodeGen(Codegen& context) {
...
llvm::Value* ast::Routine::CodeGen(Codegen& context) {
    std::vector<llvm::Type*> arg_types;
    for(auto it : *(this->argument_list))
        arg_types.push_back(it->type->toLLVMType()); 
    auto f_type = llvm::FunctionType::get(this->isProcedure() ? llvm::Type::getVoidTy(TheContext) : this->return_type->toLLVMType(), llvm::makeArrayRef(arg_types), false);
    auto function = llvm::Function::Create(f_type, llvm::GlobalValue::InternalLinkage, this->routine_name->name.c_str(), context.module);
    auto block = llvm::BasicBlock::Create(TheContext, "entry", function,NULL);
    ...
    if (this->isFunction()) {
        ...
        auto load_ret = new llvm::LoadInst(context.getValue(this->routine_name->name)->getType(), context.getValue(this->routine_name->name), "", false, context.currentBlock());
        llvm::ReturnInst::Create(TheContext, load_ret, context.currentBlock());
    }
    else if(this->isProcedure()) {
        std::cout << "Generating return void for procedure" << std::endl;
        llvm::ReturnInst::Create(TheContext, context.currentBlock());
...
llvm::Value* ast::FuncCall::CodeGen(Codegen& context) {
...
llvm::Value* ast::ProcCall::CodeGen(Codegen& context) {
    auto function = context.module->getFunction(this->id->name.c_str());
    if (function == nullptr)
...
llvm::Value* ast::SysFuncCall::CodeGen(Codegen& context) {}

// kedua fungsi di bawah merupakan kode sementara, TODO: perbaiki Sys Write
llvm::Value* ast::SysProcCall::SysProc_write(Codegen& context, bool writeln) {
  return nullptr;
}

llvm::Value* ast::SysProcCall::CodeGen(Codegen& context) {
  return nullptr;
}
...
//
    //auto func_type = llvm::FunctionType::get(llvm::Type::getVoidTy(TheContext), true);
...
//llvm::Function * Create( llvm::FunctionType *Ty, llvm::GlobalValue::LinkageTypes Linkage, const llvm::Twine &N, llvm::Module *M )
    ...
    //auto block = llvm::BasicBlock::Create(TheContext, "entry", write_func, 0);
    //context.pushBlock(block);
    ...
    for(auto arg : *argument_list) {
        ...
        if (arg_val->getType() == llvm::Type::getInt32Ty(TheContext)) {
            printf_format += "%d";     
            std::cout << "SysFuncCall write variable previous name" << arg_val->getName().str() << std::endl;
            printf_args.push_back(arg_val);
        } else if (arg_val->getType()->isDoubleTy() { // } else if (arg_val->getType()->isDoubleTy() == llvm::Type::getDoubleTy(TheContext)) {
            printf_format += "%lf";
            printf_args.push_back(arg_val);
        } else if (arg_val->getType() == llvm::Type::getInt8PtrTy(TheContext)) {
    ...
    auto printf_format_const = llvm::ConstantDataArray::getString(TheContext, printf_format.c_str());

    auto format_string_var = new llvm::GlobalVariable(*context.module, llvm::ArrayType::get(llvm::IntegerType::get(TheContext, 8), printf_format.size() + 1), true, llvm::GlobalValue::PrivateLinkage, printf_format_const, ".str");
    
    auto zero = llvm::Constant::getNullValue(llvm::IntegerType::getInt32Ty(TheContext));    
    ...
    auto var_ref = llvm::ConstantExpr::getGetElementPtr(format_str_var->getType(), indices,);

    printf_args.insert(printf_args.begin(), var_ref);
    auto call = llvm::CallInst::Create(Codegen::printf, llvm::makeArrayRef(printf_args), "", context.currentBlock());
...
* End of hapus sementara
*/
llvm::Value* ast::TypeDecl::CodeGen(Codegen& context) {}

llvm::Value* ast::Expression::CodeGen(Codegen& context) {}

llvm::Value* ast::IfStmt::CodeGen(Codegen& context) {
    Value* test = condition->CodeGen( context );
    BasicBlock *btrue = BasicBlock::Create(TheContext, "thenStmt", context.currentFunction);
    BasicBlock *bfalse = BasicBlock::Create(TheContext, "elseStmt", context.currentFunction);
    BasicBlock *bmerge = BasicBlock::Create(TheContext, "mergeStmt", context.currentFunction);    
...
llvm::Value* ast::WhileStmt::CodeGen(Codegen& context) {
    BasicBlock *sloop = BasicBlock::Create(TheContext, "startloop", context.currentFunction);
    BasicBlock *bloop = BasicBlock::Create(TheContext, "loopStmt", context.currentFunction);
    BasicBlock *bexit = BasicBlock::Create(TheContext, "eixtStmt", context.currentFunction);    
...
llvm::Value* ast::RepeatStmt::CodeGen(Codegen& context) {
    BasicBlock *bloop = BasicBlock::Create(TheContext, "loopStmt", context.currentFunction);
    BasicBlock *bexit = BasicBlock::Create(TheContext, "eixtStmt", context.currentFunction);    
...
llvm::Value* ast::ForStmt::CodeGen(Codegen& context) {
    BasicBlock *sloop = BasicBlock::Create(TheContext, "startloop", context.currentFunction);
    BasicBlock *bloop = BasicBlock::Create(TheContext, "loopStmt", context.currentFunction);
    BasicBlock *bexit = BasicBlock::Create(TheContext, "eixtStmt", context.currentFunction);    
//  initial for   
...
llvm::Value* ast::CaseStmt::CodeGen(Codegen& context){
...
llvm::Value* ast::SwitchStmt::CodeGen(Codegen& context) {
    BasicBlock* bexit = BasicBlock::Create(TheContext, "exit", context.currentFunction);

    std::vector<BasicBlock *> bblocks;
    cout<<(*list)[1]->condition<<endl;
   
    for (int i=0;i<(list->size());i++){ 
        auto bblock = BasicBlock::Create(TheContext, "caseStmt", context.currentFunction);
    ...
    {    
        ...
        BasicBlock* bnext = BasicBlock::Create(TheContext, "next", context.currentFunction);
...
llvm::Value* ast::GotoStmt::CodeGen(Codegen& context){
    llvm::Value* test= (new BooleanType("true"))->CodeGen(context);
    BasicBlock* bafter = BasicBlock::Create(TheContext, "afterGoto", context.currentFunction);
    auto ret = llvm::BranchInst::Create(context.labelBlock[label],context.currentBlock());
    context.pushBlock(bafter);
    return ret;
}
llvm::Value* ast::LabelStmt::CodeGen(Codegen& context){
...
llvm::Value* ast::ArrayType::CodeGen(Codegen& context) {
}

llvm::Value* ast::ArrayRef::CodeGen(Codegen& context) {
    return new llvm::LoadInst(this->getRef(context)->getType(), this->getRef(context), "", false, context.currentBlock());
}

llvm::Value* ast::ArrayRef::getRef(Codegen& context) {
 ...
 //   auto a_type = llvm::ArrayType::get(llvm::Type::getInt32Ty(TheContext), 9);
...
//    vec.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 0));
//    vec.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 1));
...
//    //return ret;
    ...
    idx_list.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 0));
    //idx_list.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(TheContext), 0));
    DBVAR("before codgen");
