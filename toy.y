...
%token IF THEN ELSE WHILE DO BEGINN END CASE DOWNTO FOR GOTO UINTEGER LB RB NOT
%token SYS_BOOL SF_ABSOLUTE WRITE SYS_TYPE CONST STRING REAL ARRAY CHAR
...
%type <debug> NOT OR AND MOD OF READ REPEAT TO UNEQUAL UNTIL RB
%type <debug> SF_ABSOLUTE WRITE SYS_TYPE CHAR CONST STRING REAL ARRAY VAR TYPE 
%type <debug> RIGHTP LEFTP DOWNTO FOR GOTO UINTEGER PROCEDURE RECORD END CASE SYS_BOOL
...
const_value:
	UINTEGER 									{ $$ = new ast::IntegerType(atoi($1)); $$->debug = $1; }
...
simple_type_decl:
	...
	| UINTEGER DOT DOT UINTEGER 			        { $$ = new ast::TypeDecl(new ast::RangeType(atoi($1), atoi($4))); DBMSG("reach here, + to +");}
	...
	| MINUS UINTEGER DOT DOT UINTEGER		        { $$ = new ast::TypeDecl(new ast::RangeType(-atoi($1), atoi($5))); }
	| MINUS UINTEGER DOT DOT MINUS UINTEGER	    { $$ = new ast::TypeDecl(new ast::RangeType(-atoi($1), -atoi($2))); }
...
stmt: 
	non_label_stmt 								{ $$ = $1; }
	| UINTEGER  COLON  non_label_stmt { 
		Codegen::labels.push_back(atoi($1));
...
proc_stmt : 
	...
	|  WRITE									{ $$ = new ast::SysProcCall(new ast::Identifier($1)); }
	|  WRITE  LEFTP  expression_list  RIGHTP { $$ = new ast::SysProcCall(new ast::Identifier($1), $3); }
...
;
goto_stmt : 
	GOTO  UINTEGER 								{ $$ = new ast::GotoStmt(atoi($2));}
;
...
factor: 
	...
	|  SF_ABSOLUTE 								{ $$ = new ast::SysFuncCall(new ast::Identifier($1)); }
        |  SF_ABSOLUTE LEFTP expression_list RIGHTP 	{ $$ = new ast::SysFuncCall(new ast::Identifier($1), $3); }
	|  const_value 								{ $$ = $1; };
